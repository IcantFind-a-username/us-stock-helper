...
export function PriorityAlertCard({ alert, onPress, onOpenEvidence }: PriorityAlertCardProps) {
  return (
    <View accessibilityLabel="优先提醒，演示" style={styles.card}>
      ...
      <Text style={styles.priority}>最高优先级提醒</Text>
      ...
      <Text style={styles.detail}>来源覆盖：{alert.sourceCoverage}</Text>
      <Text style={styles.detail}>更新时间：{alert.updatedAt}</Text>
      {alert.adviserAdjustment === null ? null : <Text style={styles.detail}>顾问有限调整 {alert.adviserAdjustment >= 0 ? "+" : ""}{alert.adviserAdjustment} · 不能独立触发</Text>}
      <Text style={styles.label}>失效条件</Text>
      <Text style={styles.detail}>{alert.invalidation}</Text>
      <View style={styles.actions}>
        <Pressable accessibilityHint="打开此提醒的演示引用" accessibilityLabel={`查看 ${alert.symbol} 提醒证据：当前状态 ${alert.currentState}，证据 ${alert.evidenceCount}，反证 ${alert.counterEvidenceCount}，新鲜度 ${freshnessLabel[alert.sourceFreshness]}，来源覆盖 ${alert.sourceCoverage}`} accessibilityRole="button" onPress={() => onOpenEvidence(`${alert.symbol} 提醒证据`, alert.citations.map((citation) => citation.id))} style={styles.evidenceButton}>
          <Text style={styles.evidenceText}>查看证据</Text>
        </Pressable>
        <Pressable accessibilityHint="前往股票详情" accessibilityLabel={`查看 ${alert.symbol} 提醒详情：${alert.title}，当前状态 ${alert.currentState}，新鲜度 ${freshnessLabel[alert.sourceFreshness]}，来源覆盖 ${alert.sourceCoverage}`} accessibilityRole="button" onPress={onPress} style={styles.detailButton}>
...
const styles = StyleSheet.create({
  ...
  priority: { color: colors.red, fontSize: 12, fontWeight: "800" },
