...
- [ ] **P0** `services/analysis_api`：`tests/test_adviser_briefing.py::test_a_timeout_degrades_explicitly` 直接 `import anthropic` / `import httpx` 来构造假的超时异常，违反了 `adviser_llm/pyproject.toml` 明确写下的架构原则（"adviser_llm 是仓库里唯一允许有第三方依赖的包"）——`analysis_api` 的 `dependencies = []` 是刻意的边界。2026-08-20 发现：这个测试因此变成"能不能 import 全看 CI 这一轮 pip 有没有恰好装到 anthropic/httpx"，纯属偶然（`anthropic>=0.116` 是开放版本区间，`analysis_api` 从未声明这个依赖）；CI 在合并 PR #1 时的那次运行碰巧全绿，PR #2（纯文档改动）触发的下一次运行就因 `ModuleNotFoundError: No module named 'httpx'` 变红——同一份代码、同一个测试，结果不确定。完成标准：不是给 `analysis_api` 补依赖声明（那会正式破坏架构边界），而是让这个测试不再需要真实的 anthropic/httpx 包——用一个本地测试替身（stub 异常类/对象）模拟"超时"这个可观测行为，或者把这类需要真实 SDK 异常类型的测试搬到 `adviser_llm` 自己的测试套件里。修完后验证：`analysis_api` 在全新虚拟环境里只装它自己声明的 `dependencies = []` 也能让这个测试稳定通过。
