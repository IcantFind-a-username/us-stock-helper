...
### Task 11: Served-copy Chinese sweep (Franz review finding, 2026-08-15)
Real-mode stock and dashboard screens leak code-log-style English from the server's prose channels (e.g. "Scored on 70% of the factor weight; the rest has no source yet.", factor identifiers, degradation reasons). Audit EVERY human-readable string that can reach a real-mode screen — decision notes, score/forecast explanations, unavailable-factor names, section missingReasons, brief sourceGaps — and convert each to investor-readable Chinese following the house precedent (1728910): server-side prose becomes Chinese (exact-pinned by tests, consistent with the mobile serverVocabulary contract); stable machine codes keep a client-side Chinese mapping (extending describeMarketError/gateLabel style); provenance strings (timestamps, version ids, source names) stay technical but sit in labeled secondary text, never inline in a sentence. The sweep ends with a completeness test in the house style: render every real-mode screen state with fixture payloads and assert no unmapped English prose appears (allowlist for tickers, company names, and version ids).
